...
\begin{concetto}{\fnt{\#'} e \fnt{funcall}}
...
\begin{concetto}{\fnt{mapcar}: il map}
...
\begin{concetto}{\fnt{remove-if} e \fnt{remove-if-not}: il filter}
...
\begin{concetto}{\fnt{reduce}: il reduce/fold}
...
\begin{concetto}{\fnt{mapcan}: map + flatten}
...
\begin{attenzione}
\textbf{\fn{mapcan} è distruttivo.}
Usa \fn{nconc} per concatenare i risultati: le liste restituite dalla
funzione vengono modificate. Se la funzione ritorna liste condivise o
costanti quoted, il comportamento è indefinito.
Soluzione sicura: la funzione deve sempre restituire liste \emph{fresche}
(create con \fn{list}, \fn{copy-list}, ecc.).
\end{attenzione}
...
\begin{concetto}{\fnt{lambda}: funzione senza nome}
...
;; Costruisce un comparatore parziale
(defparameter *maggiore-di-10?* (partial #'< 10))
(funcall *maggiore-di-10?* 15)   ; => T   (10 < 15)
(funcall *maggiore-di-10?* 5)    ; => NIL (10 < 5 e' falso)
(remove-if-not *maggiore-di-10?* '(5 12 8 20 3))  ; => (12 20)
\end{lstlisting}
\end{esempio}
...
;; Usare copy-list perche' sort modifica la lista originale
(sort (copy-list '(3 1 4 1 5 9)) #'<)  ; => (1 1 3 4 5 9)
(sort (copy-list '(3 1 4 1 5 9)) #'>)  ; => (9 5 4 3 1 1)

;; sort su strutture con :key
(sort (copy-list '((luca . 78) (anna . 92) (mario . 85)))
      #'>
      :key #'cdr)
...
Completa gli esercizi \textbf{69--88} (core del capitolo) più le
estensioni \textbf{89--92}.\\[4pt]
