...
\mnota{\fn{setf} funziona qui perché usa i \emph{generalized places}:
vedi la spiegazione completa a p.~\pageref{sec:setf-places}.}
...
\begin{concetto}{\fnt{maphash}: iterare su tutte le coppie}
\begin{sintassi}
...
\detail{\fn{maphash} è progettato per side-effect: non restituisce
un valore utile. Per accumulare in una struttura esterna, \fn{setf}
è la soluzione naturale e corretta in questo contesto.}
...
\detail{Costruire una matrice per indice è un caso tipico dove
\fn{dotimes} con \fn{setf} è la soluzione più diretta e leggibile:
non c'è struttura ricorsiva da attraversare, solo un range numerico
da scorrere modificando celle in posizione.}
...
ha sempre nome, eta', citta', e vuoi che il compilatore ti aiuti.
...
;; Il wrapper (list nil) crea una cons-cell mutabile che funge
;; da contenitore: setf (car s) modifica il puntatore interno.
;; E' il pattern standard per strutture dati mutabili in CL.
