...
\begin{nota}
Non ci sono esercizi dedicati al tipo \texttt{sequence} come tale:
le funzioni \fn{map}, \fn{subseq} e \fn{concatenate} compaiono
all'interno degli esercizi sulle stringhe (137--141).
Prova i frammenti di questa sezione direttamente nel REPL
prima di proseguire.
\end{nota}
...
L'esercizio~137 (\fn{reverse-string}) usa \fn{coerce} per convertire
la stringa in lista di caratteri, invertirla e riconvertirla.
L'esercizio~138 (\fn{palindrome-p}) confronta la stringa con
la sua versione invertita.
L'esercizio~139 (\fn{palindrome-robust-p}) aggiunge
la normalizzazione: ignora maiuscole, spazi e punteggiatura
prima del confronto. Richiede \fn{remove-if} e \fn{char-equal}.
\end{workbook}
...
L'esercizio~140 (\fn{anagram-p}) confronta due stringhe
verificando che abbiano gli stessi caratteri con le stesse frequenze.
Usa \fn{sort} su \fn{coerce} oppure una frequenza con hash table.
L'esercizio~141 (\fn{count-char}) conta le occorrenze di un carattere
in una stringa: breve ma utile per consolidare \fn{char=}
e il loop su sequenze.\\[4pt]
Le funzioni di stream e file non hanno esercizi dedicati
in questo capitolo: li userai nell'esercizio~148 (progetto
gestionale), che richiede il salvataggio e il caricamento
da file con \fn{with-open-file}.
\end{workbook}
...
\begin{nota}
Non c'è un esercizio dedicato a \fn{read}/\fn{write} come tali.
Il meccanismo di serializzazione compare però nell'esercizio~148
(progetto gestionale), dove salvare e ricaricare la struttura dati
da file è uno dei requisiti espliciti.
\end{nota}
...
\begin{nota}
Non c'è un esercizio dedicato alla tokenizzazione come fase separata.
La funzione \fn{tokenizza} che hai visto qui viene usata internamente
nella sezione successiva (RPN) e può essere riutilizzata liberamente
nei tuoi esercizi.
\end{nota}
...
\keyline{}{Alla fine della sequenza il risultato è in cima allo stack.}
\end{concetto}

\begin{esempio}[Evaluatore RPN su lista di simboli e numeri]
\begin{lstlisting}
;; L'input e' una lista Lisp: numeri e simboli operatore.
;; Non serve tokenizzare: la lista e' gia' strutturata.
...
(defun rpn-eval (espressione)
  "Valuta una lista in notazione RPN.
   (rpn-eval '(3 4 +))          => 7
   (rpn-eval '(3 4 * 2 +))      => 14"
  (let ((stack '()))
    (dolist (token espressione)
      (if (numberp token)
          (push token stack)
          (let ((b (pop stack))
                (a (pop stack)))
...
l'errore. Utile per conversioni tentative.
Per una gestione più robusta degli errori vedi il Capitolo~9.
\end{nota}

\begin{workbook}
\textbf{Esercizio 142} (\fn{rpn-eval})\\[4pt]
Il workbook usa la stessa interfaccia: una lista Lisp
con numeri e simboli operatore, come \fn{'(3 4 +)}.
Implementa \fn{rpn-eval} seguendo lo schema dello stack
mostrato qui. Il vincolo esplicito è di usare una pila.\\[4pt]
Testa con \fn{(rpn-eval '(5 1 2 + 4 * + 3 -))}
che deve restituire 14.
\end{workbook}
...
\begin{workbook}
\textbf{Esercizi 143--144}\\[4pt]
L'esercizio~143 (\fn{evaluate-expression}) implementa esattamente
questo evaluator: numero~→ se stesso, lista \fn{(op a1 a2)}~→
valuta ricorsivamente gli argomenti e applica l'operatore.
Usa \fn{cond} o \fn{case} su \fn{op} per le quattro operazioni.\\[4pt]
L'esercizio~144 (\fn{mio-eval}) estende il modello con un ambiente
esplicito: aggiunge il caso simbolo (cerca nell'ambiente con \fn{assoc})
e il caso \fn{(set nome valore)} che restituisce il valore insieme
all'ambiente aggiornato. L'ambiente è una alist, esattamente come
nell'esempio di questa sezione.\\[4pt]
L'esercizio~143 è il fondamentale: fallo prima di affrontare il~144.
\end{workbook}
...
Completa gli esercizi \textbf{137--144} (core del capitolo).\\[4pt]
Priorità: \textbf{137, 138, 142, 143, 144}.
L'esercizio~137 (reverse-string) introduce \fn{coerce};
138 (palindrome-p) consolida il confronto di stringhe;
142 (rpn-eval) insegna lo stack come struttura di valutazione;
143--144 sono l'evaluator, cuore del capitolo.\\[4pt]
Gli esercizi~139--141 (palindromo robusto, anagrammi, conta-caratteri)
completano la parte sulle stringhe: falli se vuoi
padroneggiare le funzioni di sequenza prima di passare al parsing.
\end{workbook}
