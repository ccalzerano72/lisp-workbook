...
\medskip
Questo capitolo e' su qualcosa di diverso: come scrivere
...
\medskip
E' anche il capitolo piu' soggettivo. Le scelte di stile
dipendono dal contesto. Quello che segue e' un insieme
...
La regola piu' importante del buon codice funzionale
non e' specifica di Lisp: una funzione fa una cosa.

\begin{concetto}{Una funzione, una responsabilita'}
...
\keyline{}{Se il corpo e' piu' lungo di una schermata, probabilmente
contiene piu' funzioni.}

\boxsep
\detail{Questa non e' una regola sul numero di righe.
E' una regola sulla coerenza concettuale.
Una funzione di 30 righe che fa davvero una cosa
e' meglio di tre funzioni di 10 righe con responsabilita' sovrapposte.}
\end{concetto}
...
La versione a tre funzioni e' piu' lunga, ma ogni parte
e' testabile indipendentemente, riusabile separatamente
...
\keyline{Argomenti}{nell'ordine piu' naturale per l'uso parziale.
...
Il \fn{!} e' una convenzione di stile, non sintassi.}
...
Fino a qui tutto il codice e' vissuto nel package
di default, \texttt{CL-USER}.
Appena il progetto ha piu' di un file, serve un sistema
di namespace per evitare conflitti di nomi.

\begin{concetto}{Package}
\keyline{}{Un package e' uno spazio di nomi: un insieme di simboli
con un nome comune.}
...
dall'interno del package. E' il meccanismo di incapsulamento
...
La soluzione e' accedere al simbolo con il nome qualificato
...
Un progetto reale ha piu' file da compilare nell'ordine giusto.
ASDF (Another System Definition Facility) e' lo strumento
standard per descrivere questa struttura.

\begin{concetto}{ASDF: cos'e' e cosa fa}
...
La documentazione completa e' su \url{https://asdf.common-lisp.dev}.
Per la maggior parte dei progetti piccoli e medi,
la struttura mostrata sopra e' sufficiente.
\end{nota}
...
non e' solo buona pratica: e' il modo piu' rapido
...
\begin{intuizione}
Scrivere il test prima di scrivere la funzione non e'
masochismo: e' il modo piu' veloce per chiarire
...
piu' potente del semplice try/catch.
...
\fn{handler-case} e' l'equivalente diretto.
La differenza principale e' che Common Lisp distingue
tra condizioni segnalate (\fn{error}, \fn{warn}, \fn{signal})
e handler che le intercettano (\fn{handler-case},
\fn{handler-bind}).
Il sistema di condizioni e' piu' flessibile del modello
exception-based perche' permette di riprendere l'esecuzione
dal punto in cui e' stata segnalata la condizione
(restart), senza necessariamente salire nello stack.
\end{confronto}

\begin{attenzione}
\fn{ignore-errors} e' comodo ma pericoloso se usato
...
puo' accadere e si e' sicuri di volerla ignorare.
...
Il profiling e' lo strumento che trasforma ``questo codice
...
piu' velocita', meno controlli di tipo.}

\boxsep
\detail{Per profiling piu' dettagliato (quale funzione specificamente),
...
Il collo di bottiglia e' quasi sempre dove non ti aspetti.
\end{intuizione}
...
a generare codice piu' efficiente, non solo a validare.
Su Allegro CL e SBCL, aggiungere \fn{(type fixnum n)} a una
funzione aritmetica intera puo' ridurre l'uso di bignum boxing
...
perche' le parentesi stanno dove stanno.
...
Quella distanza e' Lisp.

\argomento{Cosa ha reso possibile il mini-REPL}

Il mini-REPL del Capitolo~8 non e' un esercizio isolato:
...
\begin{itemize}
  ...
  \item \textbf{Capitolo~7}: il backtracking come template
        per la ricerca (il resolver del Sudoku, ma anche
        la valutazione con scelta di regola).
...
Un mini-REPL con le funzioni aritmetiche di base e' un punto
...
\begin{itemize}
  ...
  \item Aggiungere \fn{cond} e \fn{let} (gia' abbozzato
...
(funzioni piccole, package, test, condizioni), e' un'ottima
palestra per tutto quello che hai imparato.

\begin{workbook}
\textbf{Usare gli esercizi gia' fatti come banco di prova:}\\[4pt]
...
\begin{itemize}
  \item Aggiungi le docstring alle funzioni piu' importanti.
  ...
  \item Misura con \fn{time} la differenza tra \fn{fib-lento}
        e \fn{fib-veloce}: e' il modo piu' diretto per
...
\medskip
\begin{itemize}
  \item \textbf{Funzioni piccole}: una funzione, una responsabilita'.
  ...
  \item \textbf{ASDF}: descrive quali file compilare e in che ordine.
        Il file \fn{.asd} e' il punto di entrata del sistema.
  ...
  \item \textbf{Condizioni}: \fn{error}, \fn{warn},
        ...
        Piu' flessibili delle eccezioni Java grazie ai restart.
  \item \textbf{Profiling}: \fn{time} per misurare. Prima corretto,
        poi veloce. Il collo di bottiglia e' quasi sempre
        dove non ti aspetti.
\end{itemize}
...
Il resto e' pratica: scrivere codice, leggerlo, migliorarlo.
Il REPL ti aspetta.
\end{tcolorbox}
