...
All'esercizio~148 costruirai un sistema gestionale completo:
strutture dati, operazioni CRUD, salvataggio su file,
package, test automatici e gestione degli errori.

Quella distanza è Lisp.

\argomento{Cosa ha reso possibile l'esercizio~148}

Il progetto gestionale non è un esercizio isolato:
usa ogni strato del percorso:

\begin{itemize}
  \item \textbf{Capitolo~2}: \fn{defun}, \fn{defparameter},
        le funzioni come blocchi di costruzione.
  \item \textbf{Capitolo~3}: ricorsione e liste per attraversare
        l'archivio.
  \item \textbf{Capitolo~4}: higher-order per il filtraggio
        e l'ordinamento (\fn{remove-if-not}, \fn{sort}).
  \item \textbf{Capitolo~5}: \fn{defstruct} o CLOS per il modello
        dati; hash table come archivio principale.
  \item \textbf{Capitolo~8}: \fn{with-open-file}, \fn{write}
        e \fn{read} per il salvataggio e il caricamento da file.
  \item \textbf{Questo capitolo}: package con \fn{defpackage},
        test con \fn{assert}, \fn{handler-case} per la gestione
        degli errori (file non trovato, record duplicato).
\end{itemize}

\argomento{Cosa richiede l'esercizio~148}

Il workbook è esplicito sui requisiti:
modello dati con \fn{defstruct} o CLOS, hash table come archivio,
operazioni CRUD complete, filtraggio e ordinamento,
salvataggio e caricamento da file, docstring su ogni funzione pubblica,
package dedicato, almeno cinque test automatici,
nessuna variabile globale mutabile salvo l'archivio dichiarato
con \fn{defparameter}, gestione di almeno un errore con \fn{handler-case}.

Ogni requisito corrisponde a qualcosa che hai già imparato.
Il progetto non introduce concetti nuovi: ti chiede di combinarli.

\argomento{Da qui in avanti}

Il gestionale dell'esercizio~148 è un punto di partenza,
non un punto di arrivo.
Le direzioni naturali per estenderlo:

\begin{itemize}
  \item Aggiungere un sistema di autenticazione con closure
        che mantengono lo stato dell'utente corrente.
  \item Aggiungere un layer di persistenza più robusto
        con \fn{write}/\fn{read} e transazioni atomiche.
  \item Organizzare il codice con ASDF e separare
        modello, logica, I/O e test in file distinti.
  \item Aggiungere condizioni personalizzate con \fn{define-condition}
        per sostituire i semplici \fn{error} con segnali tipizzati.
\end{itemize}

\begin{workbook}
\textbf{Usare gli esercizi già fatti come banco di prova:}\\[4pt]
Non ci sono nuovi esercizi per questo capitolo.
Invece, riprendi il codice degli esercizi 124--147
e applicaci i principi di questo capitolo:

\begin{itemize}
  \item Aggiungi docstring a ogni funzione pubblica che hai scritto.
  \item Organizza l'evaluator (es.~143--144) in un package
        con \fn{defpackage} e \fn{export} per le funzioni principali.
  \item Scrivi una suite di test \fn{assert} per le funzioni
        di backtracking del Capitolo~7 (es.~124--127).
  \item Misura con \fn{time} la differenza tra la versione
        di \fn{fib-lento} e la versione con memoization
        dell'esercizio~91: è il modo più diretto per vedere
        cosa fa il profiling.
  \item Nell'esercizio~148, usa \fn{handler-case} per gestire
        almeno il caso ``file non trovato'' durante il caricamento.
\end{itemize}
\end{workbook}
