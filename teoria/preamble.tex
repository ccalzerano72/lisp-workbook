% =============================================================
%  PREAMBLE — Common Lisp: Fondamenti  (testo teorico)
...
% --- Paragrafi: no rientro, spaziatura tra paragrafi ---
...
\lstdefinestyle{lispstyle}{%
  ...
  columns=fullflexible,        % no micro-spazi, copia/incolla fedele
...
\newcommand{\fn}[1]{\texttt{\bfseries\color{clKeyword}#1}}
% Versione per titoli su sfondo colorato (eredita il colore del contesto)
\newcommand{\fnt}[1]{\texttt{\bfseries #1}}
...
\newenvironment{concetto}[1]{%
  ...
  \begin{tcolorbox}[
    ...
    before skip=14pt plus 4pt, after skip=14pt plus 3pt,
    parskip=4pt
  ]
...
% Frase-chiave: barra arancione + etichetta + testo
% \keyline{etichetta}{testo} -- etichetta sffamily + testo in nero
% \keyline{}{testo}          -- solo testo
\newcommand{\keyline}[2]{%
  \par\vspace{2pt}%
  \noindent%
  \setlength{\hangindent}{14pt}%
  \setlength{\hangafter}{1}%
  \textcolor{csOrange}{\rule{3pt}{0.9em}}\hspace{5pt}%
  \ifx&#1&%
    {\small\color{black!80} #2}%
  \else%
    {\normalsize\sffamily\bfseries\color{csBlue} #1}\enspace%
    {\small\color{black!80} #2}%
  \fi\par%
  \vspace{2pt}%
}
...
\usepackage[
  ...
  urlcolor=csTeal,
  citecolor=csGreen,
  pdftitle={Common Lisp - Fondamenti},
...
% TikZ per diagrammi (alberi, cons-cell, stack)
\usepackage{tikz}
\usetikzlibrary{trees, arrows.meta, positioning, shapes.geometric, fit, backgrounds}
