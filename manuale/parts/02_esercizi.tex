...
Assumere \fn{a}, \fn{b}, \fn{c} positivi e che \fn{c} sia il lato maggiore.}
\nopagebreak
\begin{lstlisting}
...
% ---- Nota tecnica: rimando al testo teorico ----
\label{nota:higherorder}
\begin{tcolorbox}[
  enhanced,
  colframe=black!35,
  colback=black!3,
  fonttitle=\bfseries\small,
  title={Nota --- Higher-order, closure e funzioni di prima classe},
  arc=3pt,
  boxrule=0.6pt,
...
Una \textbf{funzione di ordine superiore} (higher-order) riceve funzioni
come argomenti o ne restituisce una. In Common Lisp le funzioni sono
valori di prima classe: \fn{\#'nome} prende il riferimento,
\fn{funcall} la chiama, \fn{apply} la chiama espandendo una lista
come argomenti.

\smallskip
\begin{tabular}{@{}l@{\;\;---\;\;}l@{}}
\fn{mapcar}        & applica \fn{f} a ogni elemento, ritorna lista risultati \\
\fn{remove-if-not} & mantiene gli elementi per cui il predicato e' vero \\
\fn{remove-if}     & rimuove gli elementi per cui il predicato e' vero \\
\fn{reduce}        & riduce una lista a un valore con una funzione binaria \\
\fn{apply}         & chiama \fn{f} con gli elementi di una lista come argomenti \\
\fn{mapcan}        & come \fn{mapcar} ma concatena i risultati (flatMap) \\
\end{tabular}

\medskip
Una \textbf{closure} e' una funzione che cattura variabili
dall'ambiente lessicale in cui e' stata creata.
Le variabili catturate sopravvivono alla funzione che le ha create
e rimangono accessibili a ogni chiamata successiva.
...
(funcall *c*)   ; -> 1
(funcall *c*)   ; -> 2   (n persiste tra le chiamate)
\end{lstlisting}

\smallskip\noindent
\textcolor{black!50}{\textit{Approfondimento ---
\fn{\#'}, \fn{funcall}, \fn{apply}, lambda, closure con stato,
composizione e applicazione parziale:
\textbf{Fondamenti}, Cap.~4 (Higher-order e closure).}}
\end{tcolorbox}
...
\exspieg{L'\textit{applicazione parziale}
fissa alcuni argomenti di una funzione, restituendo una nuova funzione
che accetta gli argomenti rimanenti. \fn{partial} deve restituire una closure che, quando chiamata con ulteriori
...
Assumere valori unici, e chiavi e valori semplici (confrontabili con \fn{eql}).\\\\
...
\vincolo{assumere che entrambi i nodi siano già presenti nel grafo.}
\begin{lstlisting}
...
; -> (A B C D)
\end{lstlisting}
\vincolo{usare l'ADT queue dell'esercizio 114.}
...
\vincolo{per semplicità, assumere che le parole siano separate solo da spazi.}
\begin{lstlisting}
...
\begin{tcolorbox}[
  ...
  colback=black!3,
  fonttitle=\bfseries\small,
  title={Nota --- Macro: trasformare codice in codice},
  arc=3pt,
  boxrule=0.6pt,
  left=12pt, right=12pt, top=8pt, bottom=8pt,
  toptitle=3pt, bottomtitle=3pt,
...
Le macro operano a livello sintattico: ricevono gli argomenti
come \textit{codice grezzo non valutato} e li riscrivono in
un'altra espressione prima che il compilatore la veda.
Questo e' radicalmente diverso da una funzione, dove gli argomenti
vengono sempre valutati prima della chiamata.

\medskip
\textbf{Strumento fondamentale: \fn{macroexpand-1}.}
Dopo aver scritto una macro, verificarne sempre l'espansione:
\begin{lstlisting}
(macroexpand-1 '(quando (> x 0) (print "ok")))
; -> (IF (> X 0) (PROGN (PRINT "ok")) NIL)
\end{lstlisting}
Se l'espansione non e' quella attesa, il comportamento sara' sbagliato.

\medskip
\textbf{Cattura di variabile.}
Ogni variabile temporanea introdotta dalla macro deve usare
\fn{gensym} per evitare conflitti con le variabili del codice
chiamante. Rilevante per l'esercizio~123.

\medskip
\textbf{Quando usare una macro.}
Solo quando una funzione non basta: valutazione controllata
(come \fn{when} o \fn{and}), nuova sintassi, eliminazione di
boilerplate strutturale. Se una funzione di ordine superiore
risolve il problema, usare quella.

\smallskip\noindent
\textcolor{black!50}{\textit{Approfondimento ---
meccanismo macro, backquote, \fn{gensym} e cattura di variabile:
\textbf{Fondamenti}, Cap.~6 (Macro).}}
\end{tcolorbox}
...
\exspieg{Una \textit{permutazione} è un ordinamento di tutti gli elementi.
Con $n$ elementi ci sono $n!$ permutazioni (es: 3 elementi → 6 permutazioni).}
\nopagebreak
\begin{lstlisting}
...
\exspieg{Una \textit{combinazione} è una selezione di \fn{k} elementi
senza considerare l'ordine. Es: da \fn{(a b c d)} prendendo 2 elementi
si ottengono 6 coppie: \fn{(a b)}, \fn{(a c)}, \fn{(a d)}, \fn{(b c)}, \fn{(b d)}, \fn{(c d)}.}
\nopagebreak
\begin{lstlisting}
...
esattamente \fn{target}.}
\vincolo{assumere che \fn{numeri} contenga solo interi positivi.}
\nopagebreak
\begin{lstlisting}
...
\exspieg{Il \textit{merge sort} è un algoritmo di ordinamento che divide
la lista a metà, ordina ricorsivamente le due metà, e le fonde in una
lista ordinata.}
\nopagebreak
\begin{lstlisting}
...
\exspieg{Il \textit{quicksort} è un algoritmo di ordinamento che sceglie
un elemento \textit{pivot}, divide la lista in elementi minori e maggiori
del pivot, e ordina ricorsivamente le due parti.}
\nopagebreak
\begin{lstlisting}
...
\exspieg{Ci sono 3 pioli (A, B, C) e \fn{n} dischi di dimensione crescente
impilati sul piolo \fn{da}. Obiettivo: spostare tutta la torre sul piolo
\fn{a}, muovendo un disco alla volta e senza mai mettere un disco più
grande sopra uno più piccolo.}
\nopagebreak
\begin{lstlisting}
(hanoi 3 'A 'C 'B)
; Sposta disco 1: A -> C
...
\exspieg{Dati oggetti con peso e valore, scegliere quali mettere nello
zaino (capacità limitata) per massimizzare il valore totale senza
superare la capacità.}
\begin{lstlisting}
(knapsack '((2 3) (3 4) (4 5) (5 6)) 5)
; -> 7
\end{lstlisting}
...
\exspieg{Trovare un percorso dalla cella \fn{start} alla cella \fn{goal},
muovendosi solo nelle 4 direzioni ortogonali (su, giù, sinistra, destra)
attraverso celle libere.}
\nopagebreak
\begin{lstlisting}
...
che nessuna ne attacchi un'altra. Due regine si attaccano se sono sulla
...
\exspieg{Riempire la griglia in modo che ogni riga, colonna e blocco
$3 \times 3$ contenga i numeri da 1 a 9 senza ripetizioni.}
...
\exspieg{Un \textit{palindromo} è una stringa che si legge uguale da
sinistra a destra e da destra a sinistra (es: "anna", "radar").}
\nopagebreak
\begin{lstlisting}
(palindrome-p "anna")    ; -> T
...
(alist di coppie \fn{(nome . valore)}) passato esplicitamente.
\fn{mio-eval} restituisce sempre una coppia \fn{(valore . nuovo-env)}.
...
\begin{itemize}[leftmargin=1.8em, itemsep=1pt, topsep=2pt]
  ...
  \item se \fn{expr} è \fn{(set nome valore)}, valutare \fn{valore},
        restituire \fn{(valore . (acons nome valore env))};
  \item le espressioni aritmetiche propagano l'env aggiornato
        tra la valutazione del primo e del secondo argomento.
\end{itemize}
Poiché l'ambiente non è mutato ma restituito, chi chiama deve
usare il \fn{cdr} del risultato per incatenare le valutazioni.}
\begin{lstlisting}
;; mio-eval restituisce (valore . nuovo-env)
(let* ((env0 '())
       (r1   (mio-eval '(set x 10) env0))  ; r1 = (10 . ((x . 10)))
       (env1 (cdr r1))                     ; propaga il nuovo env
       (r2   (mio-eval '(+ x 5) env1)))    ; r2 = (15 . ...)
  (car r2))   ; -> 15
\end{lstlisting}
...
\begin{itemize}[leftmargin=2em]
  \item \fn{defstruct} o CLOS per il modello dati;
  \item hash table o lista come archivio;
  \item operazioni CRUD: inserimento, ricerca, modifica, eliminazione;
  \item filtraggio e ordinamento;
...
\begin{itemize}[leftmargin=2em]
  \item package dedicato (\fn{defpackage} + \fn{in-package});
  \item almeno 5 test automatici (funzioni \fn{test-*} che verificano
        il comportamento e stampano PASS/FAIL);
  \item nessuna variabile globale mutabile, salvo l'archivio
        esplicitamente dichiarato con \fn{defparameter};
  \item gestione di almeno un errore con \fn{handler-case}
        (es.\ file non trovato, record duplicato).
\end{itemize}

\textbf{Struttura del file:}
\begin{lstlisting}
;;; gestionale.lisp
(defpackage :gestionale (:use :cl) (:export ...))
...
\vincolo{il progetto deve poter essere caricato da file con
\fn{:ld "gestionale.lisp"}, non solo copiato nel REPL.}
...
implementazioni principali (SBCL, Allegro CL, CCL) la applicano
sul codice \textit{compilato} quando la funzione ausiliaria è definita
con \fn{labels}. In Allegro CL la TCO è attiva con \fn{:cl} (compila
il file); con \fn{:ld} (carica il sorgente interpretato) la ricorsione
in coda \textit{potrebbe} ancora consumare stack su liste molto lunghe.
Per verificare l'assenza di stack overflow su input grandi, usare \fn{:cl}.
...
\medskip\noindent
\small\textcolor{black!50}{\textit{Approfondimento ---
per una spiegazione dettagliata di frame di attivazione, stack overflow
e come la TCO li evita, vedi il testo teorico
\textbf{Fondamenti}, Cap.~3 §\,Tail recursion e Appendice~A
