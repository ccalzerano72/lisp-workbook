...
% ---- Box HOF ----
\begin{tcolorbox}[
  enhanced, colback=black!2, colframe=black!15,
  boxrule=0.3pt, arc=2pt,
  left=6pt, right=6pt, top=5pt, bottom=5pt,
  before skip=6pt, after skip=6pt
]
\footnotesize
\textbf{Funzione di ordine superiore}\,: {\color{black!70}riceve funzioni come argomenti o ne restituisce una.}

\smallskip
\scriptsize
\begin{tabular}{@{}l@{\;\;--\;\;}l@{}}
\fn{mapcar}    & \color{black!60}{applica f a ogni elemento, ritorna lista risultati} \\
\fn{remove-if} & \color{black!60}{rimuove elementi per cui il predicato è vero} \\
\fn{reduce}    & \color{black!60}{riduce lista a un valore applicando f cumulativamente} \\
\fn{apply}     & \color{black!60}{chiama f con elementi lista come argomenti separati} \\
\fn{mapcan}    & \color{black!60}{come mapcar ma concatena le liste risultato} \\
\end{tabular}
\end{tcolorbox}
...
\footnotesize
\textbf{Closure}\,: {\color{black!70}funzione che cattura variabili dall'ambiente lessicale.}

\smallskip
\scriptsize
\begin{lstlisting}
(defun make-counter ()
  (let ((n 0))                  ; n catturata dalla lambda
    (lambda () (incf n))))
(funcall (make-counter))        ; -> 1, 2, 3...
\end{lstlisting}
\end{tcolorbox}
...
\exspieg{\textbf{Valutare un polinomio} significa calcolare il suo valore
numerico sostituendo un numero a $x$. Ad esempio, il polinomio $2x^2 + 3x + 1$
valutato in $x=2$ dà: $2 \cdot 4 + 3 \cdot 2 + 1 = 8 + 6 + 1 = 15$.
La \textit{regola di Horner} evita di calcolare le potenze riscrivendo il
polinomio in forma annidata: $2x^2 + 3x + 1 = ((2 \cdot x) + 3) \cdot x + 1$.
Così si procede da sinistra: $2 \to 2 \cdot 2 + 3 = 7 \to 7 \cdot 2 + 1 = 15$.}
\begin{lstlisting}
...
\exnota{Il \fn{!} nel nome è una convenzione per indicare che la funzione
modifica i dati esistenti invece di crearne una copia.}
\begin{lstlisting}
(defparameter *v*
...
\label{ex:106}
\begin{stdbox}{106}{Albero con liste}{2}
Rappresentare un albero binario con la notazione \fn{(valore sinistra destra)}
e implementare:
\begin{lstlisting}
(make-leaf v)
(make-node v left right)
\end{lstlisting}
\exspieg{Un nodo è semplicemente una lista di tre elementi.
\fn{(make-leaf 5)} → \fn{(5 NIL NIL)};
\fn{(make-node 5 sx dx)} → \fn{(5 sx dx)}.
Implementare anche \fn{(node-value t)}, \fn{(node-left t)},
\fn{(node-right t)} come accessor (\fn{car}, \fn{cadr}, \fn{caddr}).}
Costruire l'albero di esempio:
\begin{lstlisting}
;       5
;      / \
;     3   8
...
\label{ex:107}
\begin{colbox}{107}{Struttura nodo-albero}{2}
Alternativa con \fn{defstruct}:
\begin{lstlisting}
(defstruct nodo
  valore
  (sinistra nil)
  (destra   nil))
\end{lstlisting}
\exnota{Con \fn{defstruct} si ottengono automaticamente costruttore
e accessor tipizzati.}
...
Implementare un albero binario di ricerca usando la rappresentazione
dell'esercizio~106:
\begin{lstlisting}
