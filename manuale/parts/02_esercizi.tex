...
\exspieg{Una funzione è \textit{tail-recursive} quando la chiamata ricorsiva
è l'ultima operazione eseguita (non c'è nulla da calcolare al ritorno).
Questo permette al compilatore di riutilizzare lo stesso frame dello stack,
evitando overflow su liste lunghe.
Usare \fn{labels} con un parametro accumulatore \fn{acc} inizialmente \fn{NIL}:
ad ogni passo anteporre la testa di \fn{lst} ad \fn{acc}, finché \fn{lst} è vuota.}
\begin{lstlisting}
(reverse* '(a b c d))   ; -> (D C B A)
\end{lstlisting}
